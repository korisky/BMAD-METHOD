...
git checkout --ours src/modules/bmm/sub-modules/beads/
...
- `src/bmm/workflows/` - Workflow content
- `src/bmm/agents/` - Agent definitions
- `src/core/` - Core framework
- `tools/cli/installers/` - **CRITICAL** installer code
- `tools/cli/lib/` - CLI libraries
- `tools/schema/` - Schema validation
...
- `version`: `6.0.0-Beta.X-beads_Y.Y.Y`
...
- `src/modules/bmm/sub-modules/beads/` - Beads integration
...
```bash
git diff main..HEAD -- src/bmm/workflows/
# Update Beads coordination if workflow paths changed
```
