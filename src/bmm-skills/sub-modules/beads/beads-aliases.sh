#!/bin/bash
# BMAD + Beads Integration Aliases (v0.2.0)
...
# ── Quick Reference ──────────────────────────────────────────────
#
# BMAD commands (bd_ prefix, provided by this file):
#   bd_session_start       Check HALTs, ready work, sprint status
#   bd_claim "key"         Claim story before starting
#   bd_release <id>        Release claim when done
#   bd_done "key"          Mark story/epic complete
#   bd_sync_story <file>   Sync AI-Review items to Beads tasks
#   bd_halt "reason"       HALT (priority 0 — stops all work)
#   bd_decision "title"    Track runtime decision
#   bd_blocker "title"     Track external blocker
#   bd_action "title"      Create action item
#   bd_land                Sync branches (beads-sync → main → current)
#   bd_preflight           Pre-push safety check
...
#   bd_config_sync         Configure auto-sync mode
#   bd_config_workflow     Configure workflow mode
#
# Native Beads commands (bd CLI, no underscore):
#   bd list                List all items
#   bd list --type blocker --status open   Show blockers
#   bd list --type decision                Show decisions
#   bd list --priority 0 --status open     Show HALTs (P0)
#   bd list --status in_progress           Show active work
#   bd ready               Show ready work
#   bd create --type X     Create item (blocker/decision/task)
...
bd_claim() {
  if [ -z "$BMAD_SESSION_ACTIVE" ]; then
    echo "ℹ️  Tip: run bd_session_start first for HALT/blocker checks"
  fi
...
bd_session_start() {
  ...
  # Sprint status summary (if sprint-status.yaml exists)
  local sprint_file
  sprint_file=$(find . -name "sprint-status.yaml" -maxdepth 4 2>/dev/null | head -1)
  if [[ -n "$sprint_file" ]]; then
    echo "Sprint Status:"
    echo "  $(grep -c ': backlog$' "$sprint_file" 2>/dev/null || echo 0) backlog, \
$(grep -c ': ready-for-dev$' "$sprint_file" 2>/dev/null || echo 0) ready, \
$(grep -c ': in-progress$' "$sprint_file" 2>/dev/null || echo 0) active, \
$(grep -c ': review$' "$sprint_file" 2>/dev/null || echo 0) review, \
$(grep -c ': done$' "$sprint_file" 2>/dev/null || echo 0) done"
    echo ""
  fi
  ...
  case "$mode" in
    ...
    warning|*)
      if [ -t 0 ]; then
        ...
        # _bmad_should_sync_beads already confirmed sync is needed — just do it
        echo "🔄 Auto-syncing branches (beads-sync is $ahead commits ahead)..."
        bd_land
        return $?
      fi
...
echo "BMAD+Beads aliases loaded. Run 'bd list' for status or see quick reference at top of this file."
