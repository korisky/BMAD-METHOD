...
# Source the aliases
if [ -f "src/bmm-skills/sub-modules/beads/beads-aliases.sh" ]; then
    source src/bmm-skills/sub-modules/beads/beads-aliases.sh
...
if grep -q "bd_auto_land" src/bmm-skills/sub-modules/beads/install.sh; then
...
if grep -q "bd_auto_sync" src/bmm-skills/sub-modules/beads/install.sh; then
...
if grep -q "Git Sync Architecture" src/bmm-skills/sub-modules/beads/beads-reference.md; then
...
if grep -q "Sync Branches" src/bmm-skills/4-implementation/bmad-beads-handover/workflow.md; then
...
# Test 15: Check aliases file includes bd_config_sync
echo "Test 15: Check aliases for bd_config_sync"
if grep -q "bd_config_sync" src/bmm-skills/sub-modules/beads/beads-aliases.sh; then
    echo "✅ Aliases include bd_config_sync"
else
    echo "❌ Aliases missing bd_config_sync"
fi
echo ""
...
echo "  • Install in target project: bash src/bmm-skills/sub-modules/beads/install.sh"
